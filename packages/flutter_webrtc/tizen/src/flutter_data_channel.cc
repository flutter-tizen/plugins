...
{ event_sink_ = std::move(events); return nullptr;
        },
                [&](const flutter::EncodableValue* arguments)
                -> std::unique_ptr<StreamHandlerError<flutter::EncodableValue>>
{ event_sink_ = nullptr; return nullptr;
        });
...
dataChannelDict.end()) { init.maxRetransmits = GetValue<int>(
...
void FlutterDataChannel::DataChannelSend(
  ...
  if (is_binary && TypeIs<std::vector<uint8_t>>(data)) {
    ...
    string binary = std::string((const char *)buffer.data(),
(unsigned)buffer.size()); data_channel->Send(binary, true); } else { std::string
str = GetValue<std::string>(data); data_channel->Send(str, false);
  }
  result->Success();
}
...
void FlutterRTCDataChannelObserver::OnMessage(const char *buffer, int length,
  ...
  if (event_sink_ != nullptr) {
    ...
    params[EncodableValue("data")] = binary ?
EncodableValue(std::vector<uint8_t>(str.begin(), str.end())) :
EncodableValue(str); event_sink_->Success(EncodableValue(params));
  }
}
*/
}  // namespace flutter_webrtc_plugin
