...
EncodableValue(track->id().std_string()); track_info[EncodableValue("kind")] =
...
void FlutterMediaStream::GetUserVideo(const EncodableMap& constraints,
  ...
  for (int i = 0; i < nb_video_devices; i++) {
    ...
    if (sourceId != "" && sourceId == strGuidUTF8) {
      video_capturer = base_->video_device_->Create(strNameUTF8, i, width,
height, fps); break;
    }
  }
...
EncodableValue(std::string(strGuidUTF8)); audio[EncodableValue("facing")] = "";
  ...
  for (uint16_t i = 0; i < nb_audio_devices; i++) {
    ...
    audio[EncodableValue("deviceId")] =
EncodableValue(std::string(strNameUTF8)); audio[EncodableValue("facing")] = "";
...
EncodableValue(std::string(strNameUTF8)); video[EncodableValue("facing")] = i ==
1 ? "front" : "back"; video[EncodableValue("kind")] = "videoinput";
...
void FlutterMediaStream::MediaStreamDispose(
  ...
  if (!stream) {
    result->Error("MediaStreamDisposeFailed", "stream [" + stream_id + "] not
found!"); return;
  }
