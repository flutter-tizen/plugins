...
BackendKind GetSelectedBackend() {
  static const BackendKind kind = DefaultBackendForPlatform();
  return kind;
}
...
std::unique_ptr<WebViewBackend> WebViewBackendFactory::Create(
  ...
  BackendKind kind = GetSelectedBackend();
...
void WebViewBackendFactory::ShutdownEngine() {
  if (GetSelectedBackend() != BackendKind::kEwk) {
    if (g_wv_engine_initialized) {
