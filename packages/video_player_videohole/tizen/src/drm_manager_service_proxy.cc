...
typedef int (*FuncPlayerSetDrmHandle)(player_h player, player_drm_type_e drm_type,
                               int drm_handle);
typedef int (*FuncPlayerSetDrmInitCompleteCB)(player_h player, security_init_complete_cb callback, 
                                    void *user_data);
...
void* OpenMediaPlayer() { return dlopen("libcapi-media-player.so.0", RTLD_LAZY); }
...
int player_set_drm_init_data_cb(void* handle, player_h player, 
  ...
  *(void **)(&player_set_drm_init_data_cb) = dlsym(handle, "player_set_drm_init_data_cb");                                
