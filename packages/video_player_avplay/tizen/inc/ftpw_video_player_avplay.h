...
#ifndef FLUTTER_TIZEN_PLUGINS_WRAPPER_VIDEO_PLAYER_AVPLAY_H_
#define FLUTTER_TIZEN_PLUGINS_WRAPPER_VIDEO_PLAYER_AVPLAY_H_
...
typedef enum {
  ...
  PLAYER_ADAPTIVE_INFO_URL_CUSTOM = 23,
...
int ftpw_video_player_avplay_player_set_ecore_wl_display(void *player, int type,
                                                         void *ecore_wl_window,
                                                         int x, int y,
                                                         int width, int height);
int ftpw_video_player_avplay_player_set_drm_handle(void *player,
                                                   player_drm_type_e drm_type,
                                                   int drm_handle);
int ftpw_video_player_avplay_player_set_drm_init_complete_cb(
    void *player, security_init_complete_cb callback, void *user_data);
int ftpw_video_player_avplay_player_set_drm_init_data_cb(
    void *player, set_drm_init_data_cb callback, void *user_data);
int ftpw_video_player_avplay_player_get_adaptive_streaming_info(
    void *player, void *adaptive_info, int adaptive_type);
int ftpw_video_player_avplay_player_get_track_count_v2(void *player, int type,
                                                       int *pcount);
int ftpw_video_player_avplay_player_get_video_track_info_v2(
    void *player, int index, player_video_track_info_v2 **track_info);
int ftpw_video_player_avplay_player_get_audio_track_info_v2(
    void *player, int index, player_audio_track_info_v2 **track_info);
int ftpw_video_player_avplay_player_get_subtitle_track_info_v2(
    void *player, int index, player_subtitle_track_info_v2 **track_info);
int ftpw_video_player_avplay_player_set_adaptive_streaming_info(
    void *player, void *adaptive_info, int adaptive_type);
bool ftpw_video_player_avplay_device_power_is_standby(void);
int ftpw_video_player_avplay_DMGRSetData(DRMSessionHandle_t drm_session,
                                         const char *data_type,
                                         void *input_data);
int ftpw_video_player_avplay_DMGRGetData(DRMSessionHandle_t drm_session,
                                         const char *data_type,
                                         void *output_data);
void ftpw_video_player_avplay_DMGRSetDRMLocalMode(void);
DRMSessionHandle_t ftpw_video_player_avplay_DMGRCreateDRMSession(
    const char *drm_sub_type);
bool ftpw_video_player_avplay_DMGRSecurityInitCompleteCB(
    int *drm_handle, unsigned int len, unsigned char *pssh_data,
    void *user_data);
int ftpw_video_player_avplay_DMGRReleaseDRMSession(
