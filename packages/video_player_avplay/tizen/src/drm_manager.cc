...
bool DrmManager::CreateDrmSession(int drm_type, bool local_mode) {
  if (local_mode) {
    ftpw_video_player_avplay_DMGRSetDRMLocalMode();
  }
  ...
  drm_session_ =
      ftpw_video_player_avplay_DMGRCreateDRMSession(sub_type.c_str());
  if (!drm_session_) {
  ...
  int ret = ftpw_video_player_avplay_DMGRSetData(
...
void DrmManager::StopDrmSession() {
  ...
  ret = ftpw_video_player_avplay_DMGRSetData(drm_session_, "Finalize", nullptr);
...
void DrmManager::ReleaseDrmSession() {
  ...
  int ret = ftpw_video_player_avplay_DMGRReleaseDRMSession(drm_session_);
  if (ret != DM_ERROR_NONE) {
...
bool DrmManager::GetDrmHandle(int *handle) {
  if (drm_session_) {
    *handle = 0;
    int ret = ftpw_video_player_avplay_DMGRGetData(drm_session_, "drm_handle",
                                                   handle);
    if (ret != DM_ERROR_NONE) {
...
bool DrmManager::SecurityInitCompleteCB(int *drm_handle, unsigned int len,
  ...
  return ftpw_video_player_avplay_DMGRSecurityInitCompleteCB(
...
int DrmManager::SetChallenge(const std::string &media_url) {
  ...
  ret = ftpw_video_player_avplay_DMGRSetData(
  ...
  ret =
      ftpw_video_player_avplay_DMGRSetData(drm_session_, "Initialize", nullptr);
  if (ret != DM_ERROR_NONE) {
