...
RestoreMessage::RestoreMessage(int64_t player_id) : player_id_(player_id) {}

RestoreMessage::RestoreMessage(int64_t player_id, const std::string* url,
                               const int64_t* resume_time)
    : player_id_(player_id),
      url_(url ? std::optional<std::string>(*url) : std::nullopt),
      resume_time_(resume_time ? std::optional<int64_t>(*resume_time)
                               : std::nullopt) {}

int64_t RestoreMessage::player_id() const { return player_id_; }

void RestoreMessage::set_player_id(int64_t value_arg) {
  player_id_ = value_arg;
}

const std::string* RestoreMessage::url() const {
  return url_ ? &(*url_) : nullptr;
}

void RestoreMessage::set_url(const std::string_view* value_arg) {
  url_ = value_arg ? std::optional<std::string>(*value_arg) : std::nullopt;
}

void RestoreMessage::set_url(std::string_view value_arg) { url_ = value_arg; }

const int64_t* RestoreMessage::resume_time() const {
  return resume_time_ ? &(*resume_time_) : nullptr;
}

void RestoreMessage::set_resume_time(const int64_t* value_arg) {
  resume_time_ = value_arg ? std::optional<int64_t>(*value_arg) : std::nullopt;
}

void RestoreMessage::set_resume_time(int64_t value_arg) {
  resume_time_ = value_arg;
}

EncodableList RestoreMessage::ToEncodableList() const {
  EncodableList list;
  list.reserve(3);
  list.push_back(EncodableValue(player_id_));
  list.push_back(url_ ? EncodableValue(*url_) : EncodableValue());
  list.push_back(resume_time_ ? EncodableValue(*resume_time_)
                              : EncodableValue());
  return list;
}

RestoreMessage RestoreMessage::FromEncodableList(const EncodableList& list) {
  RestoreMessage decoded(std::get<int64_t>(list[0]));
  auto& encodable_url = list[1];
  if (!encodable_url.IsNull()) {
    decoded.set_url(std::get<std::string>(encodable_url));
  }
  auto& encodable_resume_time = list[2];
  if (!encodable_resume_time.IsNull()) {
    decoded.set_resume_time(std::get<int64_t>(encodable_resume_time));
  }
  return decoded;
}
...
EncodableValue PigeonInternalCodecSerializer::ReadValueOfType(
  ...
  switch (type) {
    ...
    case 146: {
      return CustomEncodableValue(RestoreMessage::FromEncodableList(
          std::get<EncodableList>(ReadValue(stream))));
    }
    default:
...
void PigeonInternalCodecSerializer::WriteValue(
  ...
  if (const CustomEncodableValue* custom_value =
    ...
    if (custom_value->type() == typeid(RestoreMessage)) {
      stream->WriteByte(146);
      WriteValue(
          EncodableValue(
              std::any_cast<RestoreMessage>(*custom_value).ToEncodableList()),
          stream);
      return;
    }
  }
      ...
      channel.SetMessageHandler(
          [api](const EncodableValue& message,
            ...
            try {
              ...
              const auto& msg_arg = std::any_cast<const PlayerMessage&>(
                  std::get<CustomEncodableValue>(encodable_msg_arg));
              std::optional<FlutterError> output = api->Suspend(msg_arg);
              if (output.has_value()) {
                reply(WrapError(output.value()));
                return;
              }
      ...
      channel.SetMessageHandler(
          [api](const EncodableValue& message,
            ...
            try {
              ...
              const auto& msg_arg = std::any_cast<const RestoreMessage&>(
                  std::get<CustomEncodableValue>(encodable_msg_arg));
              std::optional<FlutterError> output = api->Restore(msg_arg);
              if (output.has_value()) {
                reply(WrapError(output.value()));
                return;
              }
