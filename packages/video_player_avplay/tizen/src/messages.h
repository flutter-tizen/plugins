...
class RestoreMessage {
 public:
  // Constructs an object setting all non-nullable fields.
  explicit RestoreMessage(int64_t player_id);

  // Constructs an object setting all fields.
  explicit RestoreMessage(int64_t player_id, const std::string* url,
  ...
  const int64_t* resume_time() const;
  void set_resume_time(const int64_t* value_arg);
  void set_resume_time(int64_t value_arg);

 private:
  static RestoreMessage FromEncodableList(const flutter::EncodableList& list);
  flutter::EncodableList ToEncodableList() const;
  friend class VideoPlayerAvplayApi;
  friend class PigeonInternalCodecSerializer;
  int64_t player_id_;
  std::optional<std::string> url_;
  std::optional<int64_t> resume_time_;
};
...
class VideoPlayerAvplayApi {
 public:
  ...
  virtual std::optional<FlutterError> Suspend(const PlayerMessage& msg) = 0;
  virtual std::optional<FlutterError> Restore(const RestoreMessage& msg) = 0;
